...
function withUpdateCheckQueryParameters(httpRequester, queryParameters) {
  ...
  const normalizedParams = Object.keys(queryParameters).reduce((accumulator, key) => {
  ...
  return {
    request(verb, url, requestBody, callback) {
      ...
      const targetUrl = shouldAppendParams ? appendQueryParameters(url, normalizedParams) : url;
      return httpRequester.request(verb, targetUrl, requestBody, callback);
    }
  };
}
